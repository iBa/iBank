	...
	public static boolean setup(Drivers driver, String url, iBank main) 
	{
		if(driver == Drivers.SQLite || driver == Drivers.MYSQL)
		{
			if(Drivers.SQLite.useable() || Drivers.MYSQL.useable()) 
			{
				...
				if(!db.existsTable(Configuration.Entry.DatabaseLoanTable.getValue()) || 
				   !db.existsTable(Configuration.Entry.DatabaseAccountsTable.getValue()) || 
				   !db.existsTable(Configuration.Entry.DatabaseRegionTable.getValue())
				) 
				{
					if(type == Drivers.SQLite) System.out.println("[iBank] Creating SQLite tables...");
					else if(type == Drivers.MYSQL) System.out.println("[iBank] Creating Mysql tables...");
					String sql = "";
					if(type == Drivers.SQLite)
					...
					for(String line : sql.split(";")) 
					{
						if(line.length()>1) 
							db.execute(
									line.replace("{$loan$}", 
									Configuration.Entry.DatabaseLoanTable.getValue()).replace("{$accounts$}", 
									Configuration.Entry.DatabaseAccountsTable.getValue()).replace("{$regions$}", 
									Configuration.Entry.DatabaseRegionTable.getValue())
							);
					}
				}
